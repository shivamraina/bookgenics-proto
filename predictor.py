...
def main():
    a = read_in()
    m = joblib.load('./model-data/model1.pkl')

    xtrain = np.load('./model-data/xtrain.npy', allow_pickle=True)
    xtrain = pd.Series(xtrain)

    tfidf_vectorizer = TfidfVectorizer(max_df=0.8, max_features=10000)
    xtrain_tfidf = tfidf_vectorizer.fit_transform(xtrain)

    genre_data = np.load('./model-data/genre_data.npy', allow_pickle=True)
